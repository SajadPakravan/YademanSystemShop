...
class HomePostsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cardWidth = (screenWidth * 0.92).clamp(250.0, 420.0).toDouble();
    final cardHeight = (cardWidth * 0.48).clamp(125.0, 225.0).toDouble();
    final contentHeight = (cardHeight * rows) + (spacing * (rows - 1));
    final sectionHeight = contentHeight + (verticalPadding * 2);
    final columnCount = (posts.length / rows).ceil();
                ...
                SizedBox(
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return PostHorizontalCardWidget(post: posts[index], rows: rows, length: posts.length, index: index);
                    },
                  ),
                ),
                if (viewAll != null) ...[
